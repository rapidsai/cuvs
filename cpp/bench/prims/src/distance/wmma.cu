...
__global__ void wmma_matrix_multiply(const InT* a, const InT* b, AccT* c, 
                                    int m, int n, int k, bool debug) {
    ...
    for (int i = 0; i < k; i += WMMA_K) {
        ...
        //if (i == 0 && debug == true) {
        //  printf("%d,%f,%f,%f,%f\n", threadIdx.x, a_frag.x[0], a_frag.x[1], a_frag.x[2], a_frag.x[3]);
        //}
...
static void benchmark_wmma(benchmark::State& state) {
    ...
    timer.start();
    for (auto _ : state) {
        wmma_matrix_multiply<InT, ArithT, AccT><<<gridDim, blockDim, 0, stream>>>(d_a, d_b, d_c, m, n, k, false);
    }
    timer.stop();
...
int main(int argc, char* argv[]) {
    ...
    //bench = benchmark::RegisterBenchmark("wmma_matmul/tf32", benchmark_wmma<float, wmma::precision::tf32, float>);
    //bench->Apply(CustomArguments);
