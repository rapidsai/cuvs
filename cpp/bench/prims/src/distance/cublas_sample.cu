...
__global__ void reduce_min_kernel(
  OutT* out, const AccT* z, const AccT* x_norm, const AccT* y_norm, IdxT m, IdxT n)
{
  ...
  if (threadIdx.x == 0) { out[row] = block_result; }
}
                ...
                cudaStream_t stream)
{
  ...
  reduce_min_kernel<DataT, AccT, OutT, IdxT, TPB>
    <<<blocks, TPB, 0, stream>>>(out, z, x_norm, y_norm, m, n);
}
