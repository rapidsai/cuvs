...
int main(int argc, char** argv)
{
  ...
  try {
    ...
    // Run search 21 times: 1 cold + 20 warm
    constexpr int num_warmup_runs = 1;
    constexpr int num_timed_runs  = 20;
    constexpr int total_runs      = num_warmup_runs + num_timed_runs;

    std::vector<double> search_times;
    search_times.reserve(total_runs);
    ...
    for (int run = 0; run < total_runs; run++) {
      ...
      if (run == 0) {
        std::cout << "Run " << (run + 1)
                  << " (First run - includes JIT LTO overhead): " << search_times[run] << " ms\n";
      } else {
        std::cout << "Run " << (run + 1) << ": " << search_times[run] << " ms\n";
      }
    }

    // Calculate statistics
    double first_run_time  = search_times[0];
    double avg_warmup_time = 0.0;
    for (int i = 1; i < total_runs; i++) {
      avg_warmup_time += search_times[i];
    }
    avg_warmup_time /= num_timed_runs;

    double jit_overhead = first_run_time - avg_warmup_time;
    ...
    std::cout << "Throughput (after warmup):      "
              << (params.n_queries / (avg_warmup_time / 1000.0)) << " queries/sec\n";

    // Calculate speedup after caching
    if (jit_overhead > 0) {
      double speedup = first_run_time / avg_warmup_time;
      std::cout << "Speedup after warmup:           " << speedup << "x\n";
    }
