...
{
  param.M = conf.at("M");
  if (conf.contains("efConstruction")) { param.efConstruction = conf.at("efConstruction"); } else {param.efConstruction = 40;}
}
                        ...
                        typename cuvs::bench::faiss_cpu_hnsw_flat<T>::search_param& param)
{
  ...
  if (conf.contains("check_relative_distance")) { p.check_relative_distance = conf.at("check_relative_distance"); }
  if (conf.contains("bounded_queue")) { p.bounded_queue = conf.at("bounded_queue"); }
  printf("check_relative_distance %d\n", p.check_relative_distance);
  printf("bounded_queue %d\n", p.bounded_queue);
  param.p = p;
}
