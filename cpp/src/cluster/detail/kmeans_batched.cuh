         ...
         raft::host_scalar_view<IdxT> n_iter)
{
  ...
  } else if (static_cast<IdxT>(params.batch_size) > n_samples) {
    RAFT_LOG_WARN(
  ...
  for (int seed_iter = 0; seed_iter < n_init; ++seed_iter) {
    ...
    cuvs::spatial::knn::detail::utilsbatch_load_iterator<T> data_batches(
             ...
             raft::host_scalar_view<T> inertia)
{
  ...
  for (const auto& data_batch : data_batches) {
    ...
    raft::copy(labels.data_handle() + data_batch.offset(),
