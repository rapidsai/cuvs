...
class union_find {
 public:
  ValueIdx next_label;
  std::vector<ValueIdx> parent;
  std::vector<ValueIdx> size;

  ValueIdx n_indices;

  explicit union_find(ValueIdx N_)
    : n_indices(2 * N_ - 1), parent(2 * N_ - 1, -1), size(2 * N_ - 1, 1), next_label(N_)
  {
    memset(size.data() + N_, 0, (size.size() - N_) * sizeof(ValueIdx));
  }

  auto find(ValueIdx n) -> ValueIdx
  {
    ValueIdx p;
    p = n;
  ...
  void perform_union(ValueIdx m, ValueIdx n)
  {
...
RAFT_KERNEL write_levels_kernel(const ValueIdx* children, ValueIdx* parents, ValueIdx n_vertices)
{
  ValueIdx tid = blockDim.x * blockIdx.x + threadIdx.x;
  if (tid < n_vertices) {
    ValueIdx level = tid / 2;
    ValueIdx child = children[tid];
    parents[child] = level;
  }
}
 ...
 */
template <typename ValueIdx>
RAFT_KERNEL inherit_labels(const ValueIdx* children,
                           const ValueIdx* levels,
                           std::size_t n_leaves,
                           ValueIdx* labels_,
                           int cut_level,
                           ValueIdx n_vertices)
{
  ...
  if (tid < n_vertices) {
    ValueIdx node      = children[tid];
    ValueIdx cur_level = tid / 2;
    ...
    ValueIdx cur_parent = node;
    ValueIdx label      = labels_[cur_parent];

    while (label == -1) {
      cur_parent = cur_level + n_leaves;
      cur_level  = levels[cur_parent];
      label      = labels_[cur_parent];
    }
...
struct init_label_roots {
  explicit init_label_roots(ValueIdx* labels_) : labels_(labels_) {}

  template <typename Tuple>
  __host__ __device__ void operator()(Tuple t)
  {
    labels_[thrust::get<1>(t)] = thrust::get<0>(t);
  }
 ...
 * is equal to n_clusters, then propagates the resulting labels_
 * to all the children.
 *
 * @tparam ValueIdx
 * @param handle
 ...
 * @param n_leaves
 */
template <typename ValueIdx, int tpb = 256>
void extract_flattened_clusters(raft::resources const& handle,
                                ValueIdx* labels_,
                                const ValueIdx* children,
                                size_t n_clusters,
                                size_t n_leaves)
{
  auto stream        = raft::resource::get_cuda_stream(handle);
  ...
  if (n_clusters == 1) {
    thrust::fill(thrust_policy, labels_, labels_ + n_leaves, 0);
  } else {
    /**
    ...
    thrust::device_ptr<const ValueIdx> d_ptr = thrust::device_pointer_cast(children);
    ValueIdx n_vertices = *(thrust::max_element(thrust_policy, d_ptr, d_ptr + n_edges)) + 1;
    ...
    rmm::device_uvector<ValueIdx> levels(n_vertices, stream);

    ValueIdx n_blocks = raft::ceildiv(n_vertices, static_cast<ValueIdx>(tpb));
    ...
    ValueIdx child_size = (n_clusters - 1) * 2;
    rmm::device_uvector<ValueIdx> label_roots(child_size, stream);

    ValueIdx children_cpy_start = n_edges - child_size;
    ...
    thrust::sort(thrust_policy,
                 ...
                 thrust::greater<ValueIdx>());

    rmm::device_uvector<ValueIdx> tmp_labels(n_vertices, stream);

    // Init labels_ to -1
    thrust::fill(thrust_policy, tmp_labels.data(), tmp_labels.data() + n_vertices, -1);

    // Write labels_ for cluster roots to "labels_"
    ...
    thrust::for_each(
      thrust_policy, z_iter, z_iter + n_clusters, init_label_roots<ValueIdx>(tmp_labels.data()));

    /**
     * Step 2: Propagate labels_ by having children iterate through their parents
     *     1. Initialize labels_ to -1
     *     2. For each element in levels array, propagate until parent's
     *        label is !=-1
     */
    ValueIdx cut_level = (n_edges / 2) - (n_clusters - 1);

    inherit_labels<<<n_blocks, tpb, 0, stream>>>(
      children, levels.data(), n_leaves, tmp_labels.data(), cut_level, n_vertices);

    // copy tmp labels_ to actual labels_
    raft::copy_async(labels_, tmp_labels.data(), n_leaves, stream);
  }
}
