...
#pragma omp parallel for
  ...
  // oss << "after merge global neighbors THREAD " << omp_get_thread_num();
  raft::print_device_vector(
    "\nafter merge global neighbors", global_neighbors.data_handle(), k, std::cout);
  raft::print_device_vector(
    "after merge global dists", global_distances.data_handle(), k, std::cout);
  raft::print_device_vector(
    "after merge global neighbors1", global_neighbors.data_handle() + k, k, std::cout);
  raft::print_device_vector(
    "after merge global dists1", global_distances.data_handle() + k, k, std::cout);
}
