...
void _build(cuvsResources_t res,
            cuvsVamanaIndexParams* params,
            cuvsVamanaIndex* index,
                                      ...
                                      cuvsVamanaIndex_t index)
{
  return cuvs::core::translate_exceptions([=] {
    ...
    if (dataset.dtype.code == kDLFloat && dataset.dtype.bits == 32) {
      _build<float>(res, params, index, dataset_tensor);
    } else if (dataset.dtype.code == kDLInt && dataset.dtype.bits == 8) {
      _build<int8_t>(res, params, index, dataset_tensor);
    } else if (dataset.dtype.code == kDLUInt && dataset.dtype.bits == 8) {
      _build<uint8_t>(res, params, index, dataset_tensor);
    } else {
