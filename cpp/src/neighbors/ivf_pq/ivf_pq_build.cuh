...
//   auto pq_extents = list_spec<uint32_t, IdxT>{PqBits, pq_dim, true}.make_list_extents(out_ix +
//   1); auto pq_dataset = raft::make_mdspan<uint8_t, uint32_t, raft::row_major, false, true>(
                      ...
                      std::variant<uint32_t, const uint32_t*> offset_or_indices)
{
  ...
  flat_compute_residuals<T, uint32_t>(res,
                                      ...
                                      label,
                                      mr);
  launch_encode_list_data_kernel(
    res, index, new_vectors_residual.view(), label, offset_or_indices, (IdxT)n_rows);
  // constexpr uint32_t kBlockSize  = 256;
