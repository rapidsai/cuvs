...
#define CUVS_INST_BFKNN(T)                                                                   \
  ...
  void search(raft::resources const& res,                                                    \
              ...
              std::optional<DistanceEpilogue<T>> distance_epilogue)                          \
  {                                                                                          \
    if (sample_filter.has_value() && distance_epilogue.has_value()) {                        \
      RAFT_FAIL("can't specify both a sample_filter and a distance_epilogue");               \
    }                                                                                        \
    if (!sample_filter.has_value()) {                                                        \
      detail::brute_force_search<T, int64_t>(                                                \
        res, idx, queries, neighbors, distances, std::nullopt, distance_epilogue);           \
    } else {                                                                                 \
