...
struct KeyValueBlockSelect {
  ...
  __device__ inline KeyValueBlockSelect(
    K initKVal, K initVKey, V initVVal, K* smemK, raft::KeyValuePair<K, V>* smemV, int k)
    : initK(initKVal),
      initVk(initVKey),
      initVv(initVVal),
      numVals(0),
...
#pragma unroll
  ...
  __device__ inline void mergeWarpQ()
  {
    ...
    warpSortAnyRegisters<K, raft::KeyValuePair<K, V>, NumThreadQ, !Dir, Comp>(threadK, threadV);

    constexpr int kNumWarpQRegisters = NumWarpQ / raft::WarpSize;
    K warpKRegisters[kNumWarpQRegisters];
    raft::KeyValuePair<K, V> warpVRegisters[kNumWarpQRegisters];
...
#pragma unroll
    ...
    // one sorted list
    warpMergeAnyRegisters<K,
                          raft::KeyValuePair<K, V>,
                          kNumWarpQRegisters,
                          NumThreadQ,
                          !Dir,
                          Comp,
...
#pragma unroll
  ...
  __device__ inline void reduce()
  {
    ...
    FinalBlockMerge<kNumWarps, ThreadsPerBlock, K, raft::KeyValuePair<K, V>, NumWarpQ, Dir, Comp>::
