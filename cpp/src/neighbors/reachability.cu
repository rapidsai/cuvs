...
void mutual_reachability_graph(const raft::resources& handle,
                               ...
                               float alpha)
{
  RAFT_EXPECTS(core_dists.size() == X.extents(0), "core_dists doesn't have expected size");
  RAFT_EXPECTS(indptr.size() == X.extents(0) + 1, "indptr doesn't have expected size");
