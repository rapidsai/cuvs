...
#define COMPUTE_SMEM_SIZES(degree, visited_size, DEG, CANDS)				\
	if(degree == DEG && visited_size == CANDS) {					\
	    search_smem_sort_size = static_cast<int>(sizeof(typename 			\
	    cub::BlockMergeSort<DistPair<IdxT,accT>,32,CANDS/32>::TempStorage));	\
											\
	    prune_smem_sort_size = static_cast<int>(sizeof(typename 			\
	    cub::BlockMergeSort<DistPair<IdxT,accT>,32,(CANDS+DEG)/32>::TempStorage));	\
	}										
...
#define PRUNE_CALL_SORT(degree, visited_list, DEG, CANDS)			\
	if(degree == DEG && visited_list == CANDS) {				\
  		using BlockSortT = 						\
		cub::BlockMergeSort<DistPair<IdxT,accT>,32,(DEG+CANDS)/32>;	\
  		auto& sort_mem = 						\
		reinterpret_cast<typename BlockSortT::TempStorage&>(smem);	\
  		sort_edges_and_cands<accT,IdxT,DEG,CANDS>			\
			(new_nbh_list, &query_list[i], &sort_mem);		\
	}
...
#define SEARCH_CALL_SORT(topk, CANDS)						\
	if(topk == CANDS) {							\
    	    using BlockSortT = 							\
		cub::BlockMergeSort<DistPair<IdxT,accT>,32,CANDS/32>;		\
	    auto& sort_mem = 							\
 		reinterpret_cast<typename BlockSortT::TempStorage&>(smem);	\
