...
__forceinline__ __device__ void sort_edges_and_cands(DistPair<IdxT,accT>* new_nbh_list,
  ...
  for(int i=ELTS-2; i>0; i--) {
    if(tmp[i].idx == tmp[i-1].idx) { tmp[i].idx = raft::upper_bound<IdxT>(); tmp[i].dist = raft::upper_bound<accT>(); }
  }
  if(threadIdx.x==0) {
    if(tmp[0].idx == tmp[ELTS-1].idx) { tmp[0].idx = raft::upper_bound<IdxT>(); tmp[0].dist = raft::upper_bound<accT>(); }
  }
