...
// Load candidates (from query) and previous edges (from nbh_list) into registers (tmp) spanning warp
template <typename accT, typename IdxT = uint32_t, int DEG, int CANDS>
__forceinline__ __device__ void load_to_registers(
				      ...
				      DistPair<IdxT,accT>* nbh_list)
{
    ...
    for(int i=0; i<cands_per_thread; i++) {
      ...
      if(cands_per_thread*threadIdx.x+i >= query->size) {
        tmp[i].idx = INFTY<IdxT>();
	tmp[i].dist = INFTY<accT>();
      }
    }
...
__forceinline__ __device__ void sort_edges_and_cands(DistPair<IdxT,accT>* new_nbh_list,
		  QueryCandidates<IdxT,accT>* query,
  ...
  if(tmp[ELTS-1].idx == tmp[ELTS-2].idx) {
    new_nbh_list[ELTS*threadIdx.x + (ELTS-1)].idx = INFTY<IdxT>();
    new_nbh_list[ELTS*threadIdx.x + (ELTS-1)].dist = INFTY<accT>();
  }
  __shfl_up_sync(0xffffffff, tmp[ELTS-1].idx, 1);
  __syncthreads();

  for(int i=ELTS-2; i>0; i--) {
    if(tmp[i].idx == tmp[i-1].idx) { tmp[i].idx = INFTY<IdxT>(); tmp[i].dist = INFTY<accT>(); }
  }
  if(threadIdx.x==0) {
    if(tmp[0].idx == tmp[ELTS-1].idx) { tmp[0].idx = INFTY<IdxT>(); tmp[0].dist = INFTY<accT>(); }
  }
...
__global__ void RobustPruneKernel(
    ...
    raft::mdspan<const T, raft::matrix_extent<int64_t>, raft::row_major, Accessor> dataset,
     void* query_list_ptr,
     int num_queries, int degree, int visited_size, int n,
     cuvs::distance::DistanceType metric, float alpha, int dim,
     int sort_smem_size) {
  ...
  DistPair<IdxT,accT>* new_nbh_list = reinterpret_cast<DistPair<IdxT,accT>*>(&smem[dim*sizeof(T) + sort_smem_size]);
  ...
  for(int i=blockIdx.x; i<num_queries; i+=gridDim.x) {
    ...
    for(int j=0; j<degree; j++) {
      if(new_nbh_list[j].idx != INFTY<IdxT>()) {
        new_nbh_list[j].dist = dist<T,accT>(s_query.coords, &vec_ptr[(size_t)new_nbh_list[j].idx*(size_t)dim], dim, metric);
      }
      else {
