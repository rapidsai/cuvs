...
auto clone(const raft::resources& res, const index<IdxT>& source) -> index<T, IdxT>
{
                  ...
                  uint32_t dim) -> index<T, IdxT>
{
  ...
  // Train the kmeans clustering
  {
    ...
    rmm::device_uvector<float> float_trainset(
      n_rows_train * index.dim() * 8, stream, raft::resource::get_large_workspace_resource(handle));
    auto float_trainset_view = raft::make_device_matrix_view<float, IdxT>(float_trainset.data(), n_rows_train, index.dim());
    raft::linalg::map_offset(handle, float_trainset_view, binary_to_float_decode_op<IdxT>(trainset.data(), index.dim()));
    trainset.clear();
    cuvs::cluster::kmeans_balanced::fit(
      handle, kmeans_params, raft::make_const_mdspan(float_trainset_view), centers_view);
  }
