...
template <typename Index_t>
BloomFilter<Index_t>::BloomFilter(size_t nrow, size_t num_sets_per_list, size_t num_hashs)
  : nrow_(nrow),
...
template <typename Index_t>
void BloomFilter<Index_t>::add(size_t list_id, Index_t key)
{
  if (is_cleared) { is_cleared = false; }
  uint32_t hash = hash_0(key);
  size_t global_set_idx =
    list_id * num_bits_per_set_ * num_sets_per_list_ + key % num_sets_per_list_ * num_bits_per_set_;
...
template <typename Index_t>
bool BloomFilter<Index_t>::check(size_t list_id, Index_t key)
{
...
template <typename Index_t>
void BloomFilter<Index_t>::clear()
{
...
template <typename Index_t>
uint32_t BloomFilter<Index_t>::hash_0(uint32_t value)
{
  value *= 1103515245;
  value += 12345;
  value ^= value << 13;
...
template <typename Index_t>
uint32_t BloomFilter<Index_t>::hash_1(uint32_t value)
{
  value *= 1664525;
  value += 1013904223;
  value ^= value << 13;
