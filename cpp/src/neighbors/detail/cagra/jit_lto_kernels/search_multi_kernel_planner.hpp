...
struct CagraMultiKernelSearchPlanner : AlgorithmPlanner {
                                                ...
                                                uint32_t pq_len  = 0)
  {
    ...
    if (is_vpq) {
      ...
      base = "get_team_size_bitshift_vpq_t" + std::to_string(team_size);
      base += "_dim" + std::to_string(dataset_block_dim);
      base += "_" + std::to_string(pq_bits) + "pq_" + std::to_string(pq_len) + "subd";
      base += "_" + params;
      this->device_functions.push_back(base);
      ...
      base = "get_team_size_bitshift_standard_t" + std::to_string(team_size);
      base += "_dim" + std::to_string(dataset_block_dim);
      base += "_" + params;
      this->device_functions.push_back(base);
 ...
 private:
  ...
  static std::string build_entrypoint_name(const std::string& kernel_name,
                                           ...
                                           uint32_t pq_len)
  {
    // Special case: apply_filter_kernel doesn't use dataset_descriptor, so no suffixes needed
