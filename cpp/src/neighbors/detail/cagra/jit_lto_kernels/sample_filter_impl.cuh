...
template <typename SourceIndexT>
__device__ bool sample_filter_mp_bitset_impl(uint32_t /*query_id*/,
                                             SourceIndexT node_id,
                                             void* filter_data)
{
  if (filter_data == nullptr) { return true; }

  auto* data = static_cast<mp_bitset_filter_data_t<SourceIndexT>*>(filter_data);
  if (data->bitset_ptr == nullptr) { return true; }
  if (data->partition_offsets == nullptr) { return true; }

  raft::core::bitset_view<uint32_t, SourceIndexT> const view{
    data->bitset_ptr, data->bitset_len, data->original_nbits};
  const auto offset = static_cast<SourceIndexT>(data->partition_offsets[blockIdx.z]);
  return view.test(offset + node_id);
}
