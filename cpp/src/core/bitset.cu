...
_RAFT_HOST_DEVICE bitset_view<bitset_t, index_t>::bitset_view(bitset_t* bitset_ptr,
                                                              index_t bitset_len)
...
template <typename bitset_t, typename index_t>
_RAFT_HOST_DEVICE bitset_view<bitset_t, index_t>::bitset_view(
  raft::device_vector_view<bitset_t, index_t> bitset_span, index_t bitset_len)
...
template <typename bitset_t, typename index_t>
_RAFT_HOST_DEVICE index_t bitset_view<bitset_t, index_t>::n_elements() const
{
  return raft::ceildiv(bitset_len_, bitset_element_size);
}
template <typename bitset_t, typename index_t>
raft::device_vector_view<bitset_t, index_t> bitset_view<bitset_t, index_t>::to_mdspan()
{
  return raft::make_device_vector_view<bitset_t, index_t>(bitset_ptr_, n_elements());
}
template <typename bitset_t, typename index_t>
raft::device_vector_view<const bitset_t, index_t> bitset_view<bitset_t, index_t>::to_mdspan() const
{
...
template <typename bitset_t, typename index_t>
cuvs::core::bitset_view<bitset_t, index_t> bitset<bitset_t, index_t>::view()
{
  return bitset_view<bitset_t, index_t>(to_mdspan(), bitset_len_);
}
template <typename bitset_t, typename index_t>
cuvs::core::bitset_view<const bitset_t, index_t> bitset<bitset_t, index_t>::view() const
{
  return bitset_view<const bitset_t, index_t>(to_mdspan(), bitset_len_);
}
template <typename bitset_t, typename index_t>
bitset_t* bitset<bitset_t, index_t>::data()
{
  return bitset_.data();
}
template <typename bitset_t, typename index_t>
const bitset_t* bitset<bitset_t, index_t>::data() const
{
  return bitset_.data();
}

template <typename bitset_t, typename index_t>
index_t bitset<bitset_t, index_t>::size() const
{
  return bitset_len_;
}
...
template <typename bitset_t, typename index_t>
raft::device_vector_view<bitset_t, index_t> bitset<bitset_t, index_t>::to_mdspan()
{
  return raft::make_device_vector_view<bitset_t, index_t>(bitset_.data(), n_elements());
}

template <typename bitset_t, typename index_t>
raft::device_vector_view<const bitset_t, index_t> bitset<bitset_t, index_t>::to_mdspan() const
{
...
template <typename bitset_t, typename index_t>
index_t bitset<bitset_t, index_t>::count(const raft::resources& res)
{
  auto count_gpu_scalar = raft::make_device_scalar<index_t>(res, 0.0);
  count(res, count_gpu_scalar.view());
  index_t count_cpu = 0;
  raft::update_host(
    &count_cpu, count_gpu_scalar.data_handle(), 1, raft::resource::get_cuda_stream(res));
