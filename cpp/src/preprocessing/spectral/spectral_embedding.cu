               ...
               raft::device_matrix_view<float, int, raft::col_major> embedding) -> int
{
  ...
  // raft::print_device_vector("test_coo_rows", test_coo_rows.data_handle(), coo_no_zeros.nnz,
  // std::cout); raft::print_device_vector("test_coo_cols", test_coo_cols.data_handle(),
  // coo_no_zeros.nnz, std::cout); raft::print_device_vector("test_coo_vals",
  // test_coo_vals.data_handle(), coo_no_zeros.nnz, std::cout);

  raft::resource::sync_stream(handle, stream);

  auto coo_structure_view =
    raft::make_device_coordinate_structure_view<int, int, int>(test_coo_rows.data_handle(),
                                                               test_coo_cols.data_handle(),
                                                               coo_no_zeros.n_rows,
                                                               coo_no_zeros.n_cols,
                                                               coo_no_zeros.nnz);
  ...
  // auto sym_coo1_structure_view = raft::make_device_coordinate_structure_view<int, int,
  // int>(sym_coo1_rows.data_handle(), sym_coo1_cols.data_handle(), coo_no_zeros.n_rows,
  // coo_no_zeros.n_cols, coo_no_zeros.nnz * 2); auto sym_coo1_matrix_view =
  // raft::make_device_coo_matrix_view<float, int, int, int>(
  //   sym_coo1_vals.data_handle(), sym_coo1_structure_view);

  // std::cout << "coo_matrix_view.n_rows: " << coo_matrix_view.structure_view().get_n_rows() <<
  // std::endl; std::cout << "sym_coo1_matrix_view.n_rows: " <<
  // sym_coo1_matrix_view.structure_view().get_n_rows() << std::endl;

  // raft::print_device_vector("coo_no_zeros.rows", coo_no_zeros.rows(), coo_no_zeros.n_rows,
  // std::cout); raft::print_device_vector("coo_no_zeros.cols", coo_no_zeros.cols(),
  // coo_no_zeros.n_cols, std::cout); raft::print_device_vector("coo_no_zeros.vals",
  // coo_no_zeros.vals(), coo_no_zeros.nnz, std::cout);

  raft::resource::sync_stream(handle, stream);

  auto sym_coo1_matrix = raft::make_device_coo_matrix<float, int, int, int>(
    handle, coo_no_zeros.n_rows, coo_no_zeros.n_cols);
  ...
  // raft::sparse::linalg::detail::coo_symmetrize_mytest<128, float, int, int>(coo_matrix_view,
  // sym_coo1_matrix_view, reduction_op, stream);

  raft::resource::sync_stream(handle, stream);

  // raft::sparse::COO<float> sym_coo1(sym_coo1_rows, sym_coo1_cols, sym_coo1_vals, coo_no_zeros.nnz
  // * 2, coo_no_zeros.n_rows, coo_no_zeros.n_cols);  // Don't pre-allocate dimensions
  ...
  std::cout << "sym_coo1.n_rows: " << sym_coo1_matrix_view.structure_view().get_n_rows()
            << std::endl;
  std::cout << "sym_coo1.n_cols: " << sym_coo1_matrix_view.structure_view().get_n_cols()
            << std::endl;
  ...
  // sym_coo1_matrix_view.structure_view().get_nnz(), std::cout);
  // raft::print_device_vector("sym_coo1.rows", sym_coo1.rows(), sym_coo1.nnz, std::cout);

  // raft::print_device_vector("sym_coo1_cols",
  // sym_coo1_matrix_view.structure_view().get_cols().data(),
  // sym_coo1_matrix_view.structure_view().get_nnz(), std::cout);
  // raft::print_device_vector("sym_coo1.cols", sym_coo1.cols(), sym_coo1.nnz, std::cout);

  // raft::print_device_vector("sym_coo1_vals", sym_coo1_matrix_view.get_elements().data(),
  // sym_coo1_matrix_view.structure_view().get_nnz(), std::cout);
  ...
  raft::sparse::op::coo_sort<float>(coo_no_zeros.n_rows,
                                    coo_no_zeros.n_cols,
                                    sym_coo1_nnz,
                                    sym_coo1_rows,
                                    sym_coo1_cols,
  ...
  // auto sym_coo_structure = raft::make_device_coordinate_structure(handle, coo_no_zeros.n_rows,
  // coo_no_zeros.n_cols, coo_no_zeros.nnz); auto sym_coo_matrix =
  // raft::make_device_coo_matrix<float, int, int, int>(handle, coo_no_zeros.n_rows,
  // coo_no_zeros.n_cols); raft::sparse::op::detail::coo_remove_scalar_mytest<128, float, int,
  // int>(sym_coo1_matrix_view, sym_coo_matrix, 0.0F, stream);
  ...
  // raft::sparse::op::coo_sort<float>(sym_coo_n_rows, sym_coo_n_cols, sym_coo_nnz, sym_coo_rows,
  // sym_coo_cols, sym_coo_vals, stream);
  ...
  // raft::resource::sync_stream(handle, stream);
  // raft::sparse::convert::sorted_coo_to_csr(sym_coo_rows, sym_coo_nnz, sym_coo_row_ind.data(),
  // sym_coo_n_rows, stream);
  ...
  // raft::sparse::op::coo_remove_scalar<float>(sym_coo1_rows.data(), sym_coo1_cols.data(),
  ...
  // std::cout << "sym_coo1_rows.size(): " << sym_coo1_rows.size() << std::endl;
  // std::cout << "sym_coo1.n_rows: " << sym_coo1_matrix_view.structure_view().get_n_rows() <<
  // std::endl; std::cout << "sym_coo1.n_cols: " <<
  // sym_coo1_matrix_view.structure_view().get_n_cols() << std::endl; std::cout << "sym_coo1.nnz: "
  // << sym_coo1_matrix_view.structure_view().get_nnz() << std::endl;
  ...
  // raft::print_device_vector("sym_coo1_rows", sym_coo1_rows.data(), sym_coo1_rows.size(),
  // std::cout); raft::print_device_vector("sym_coo1.rows", sym_coo1.rows(), sym_coo1.nnz,
  // std::cout);
  auto zero_scalar = raft::make_host_scalar<float>(0.0f);

  auto sym_coo_matrix = raft::make_device_coo_matrix<float, int, int, int>(
    handle, coo_no_zeros.n_rows, coo_no_zeros.n_cols);
