...
{
  constexpr int kBitsPerElement = 8 * sizeof(T);
  constexpr int tile_dim_m      = kBitsPerElement;
...
#pragma unroll
    ...
    // Write output.
    int out_m = tile_idx_m / kBitsPerElement;
                      ...
                      raft::device_matrix_view<T, int, raft::layout_c_contiguous> out)
{
  ...
  RAFT_EXPECTS(raft::ceildiv(in.extent(0), kBitsPerElement) == out.extent(0),
  ...
  const int num_s_ms         = raft::getMultiProcessorCount();
  int blocks_per_sm          = 0;
  constexpr int kNumThreads  = 128;
  constexpr int kDynSmemSize = 0;
  RAFT_CUDA_TRY(cudaOccupancyMaxActiveBlocksPerMultiprocessor(
    &blocks_per_sm, compress_to_bits_kernel<T>, kNumThreads, kDynSmemSize));

  dim3 grid(num_s_ms * blocks_per_sm);
  dim3 block(128);
