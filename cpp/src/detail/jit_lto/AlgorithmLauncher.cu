...
AlgorithmLauncher::AlgorithmLauncher(CUlibrary l, CUkernel k) : library{l}, kernel{k}
{
  ...
  // }
  // std::cout << "AlgorithmLauncher constructed with kernel: " << kernel << ", library: " <<
  // library
  //           << std::endl;
}
  ...
  cudaStream_t stream, dim3 grid, dim3 block, std::size_t shared_mem, void** kernel_args)
{
  ...
  //   std::cerr << "ERROR: Invalid block dimensions: " << block.x << "x" << block.y << "x" <<
  //   block.z
  //             << std::endl;
  //   return;
  // }

  // std::cout << "Grid: " << grid.x << "x" << grid.y << "x" << grid.z << ", Block: " << block.x <<
  // "x"
  ...
  // std::cout << "Starting stream synchronization (this may hang if kernel is stuck)..." <<
  // std::endl;
  ...
  //   cuFuncGetAttribute(&max_threads, CU_FUNC_ATTRIBUTE_MAX_THREADS_PER_BLOCK,
  //   (CUfunction)kernel);
