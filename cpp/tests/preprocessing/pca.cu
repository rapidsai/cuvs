...
class PcaTest : public ::testing::TestWithParam<PcaInputs<T>> {
 ...
 protected:
  void basicTest()
  {
    ...
    auto components_view = raft::make_device_matrix_view<T, int64_t, raft::col_major>(
      components.data(), prms.n_components, params.n_col);
    auto explained_var_view =
      raft::make_device_vector_view<T, int64_t>(explained_vars.data(), prms.n_components);
    auto explained_var_ratio_view =
      raft::make_device_vector_view<T, int64_t>(explained_var_ratio.data(), prms.n_components);
    auto singular_vals_view =
      raft::make_device_vector_view<T, int64_t>(singular_vals.data(), prms.n_components);
    auto mu_view         = raft::make_device_vector_view<T, int64_t>(mean.data(), params.n_col);
    auto noise_vars_view = raft::make_device_scalar_view<T>(noise_vars.data());

    cuvs::preprocessing::pca::fit(handle,
                                  prms,
                                  input_view,
                                  components_view,
                                  explained_var_view,
    ...
    auto trans_data_view = raft::make_device_matrix_view<T, int64_t, raft::col_major>(
      trans_data.data(), params.n_row, prms.n_components);

    cuvs::preprocessing::pca::transform(
      handle, prms, input_view, components_view, singular_vals_view, mu_view, trans_data_view);

    auto data_back_view = raft::make_device_matrix_view<T, int64_t, raft::col_major>(
      data_back.data(), params.n_row, params.n_col);
  ...
  void advancedTest()
  {
    ...
    rmm::device_uvector<T> mean2(params.n_col2, stream);
    rmm::device_uvector<T> noise_vars2(1, stream);

    auto input_view = raft::make_device_matrix_view<T, int64_t, raft::col_major>(
      data2.data(), params.n_row2, params.n_col2);
    auto trans_view = raft::make_device_matrix_view<T, int64_t, raft::col_major>(
      data2_trans.data(), params.n_row2, prms.n_components);
    auto comp_view = raft::make_device_matrix_view<T, int64_t, raft::col_major>(
      components2.data(), prms.n_components, params.n_col2);
    auto ev_view =
      raft::make_device_vector_view<T, int64_t>(explained_vars2.data(), prms.n_components);
    auto evr_view =
      raft::make_device_vector_view<T, int64_t>(explained_var_ratio2.data(), prms.n_components);
    auto sv_view =
      raft::make_device_vector_view<T, int64_t>(singular_vals2.data(), prms.n_components);
    auto mu_view    = raft::make_device_vector_view<T, int64_t>(mean2.data(), params.n_col2);
    auto noise_view = raft::make_device_scalar_view<T>(noise_vars2.data());

    cuvs::preprocessing::pca::fit_transform(handle,
                                            prms,
                                            input_view,
                                            trans_view,
                                            comp_view,
    ...
    auto data2_back_view = raft::make_device_matrix_view<T, int64_t, raft::col_major>(
      data2_back.data(), params.n_row2, params.n_col2);

    cuvs::preprocessing::pca::inverse_transform(
      handle, prms, trans_view, comp_view, sv_view, mu_view, data2_back_view);
  }
