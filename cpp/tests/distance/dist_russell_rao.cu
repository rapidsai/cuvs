...
typedef DistanceRussellRao<half, float> DistanceRussellRaoH;
TEST_P(DistanceRussellRaoH, Result)
{
  int m = params.isRowMajor ? params.m : params.n;
  int n = params.isRowMajor ? params.n : params.m;
  ASSERT_TRUE(cuvs::devArrMatch(
...
INSTANTIATE_TEST_CASE_P(DistanceTests, DistanceRussellRaoH, ::testing::ValuesIn(inputsh));
