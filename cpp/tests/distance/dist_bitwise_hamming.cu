                    ...
                    bool isRowMajor)
{
  ...
  // Compute on CPU
  naive_impl(h_x.data(), h_y.data(), h_dist.data(), m, n, k, isRowMajor);
...
class BitwiseHammingTest : public ::testing::TestWithParam<BitwiseHammingInputs> {
 public:
  ...
  void SetUp() override
  {
    ...
    raft::random::uniformInt(handle, r, x.data(), params.m * params.k, uint8_t(0), uint8_t(255));
    raft::random::uniformInt(handle, r, y.data(), params.n * params.k, uint8_t(0), uint8_t(255));
    naive_distance<uint8_t, uint32_t, OutputType>(
      handle, x.data(), y.data(), dist_ref.data(), params.m, params.n, params.k, params.isRowMajor);

    // Compute optimized distance (will use uint32_t/uint64_t paths when possible)
    if (params.isRowMajor) {
      auto x_view =
        raft::make_device_matrix_view<const uint8_t, std::int64_t, raft::layout_c_contiguous>(
          x.data(), params.m, params.k);
      auto y_view =
        raft::make_device_matrix_view<const uint8_t, std::int64_t, raft::layout_c_contiguous>(
          y.data(), params.n, params.k);
      auto dist_view =
        raft::make_device_matrix_view<OutputType, std::int64_t, raft::layout_c_contiguous>(
          dist_optimized.data(), params.m, params.n);

      cuvs::distance::pairwise_distance(
        handle, x_view, y_view, dist_view, cuvs::distance::DistanceType::BitwiseHamming);
    } else {
      auto x_view =
        raft::make_device_matrix_view<const uint8_t, std::int64_t, raft::layout_f_contiguous>(
          x.data(), params.m, params.k);
      auto y_view =
        raft::make_device_matrix_view<const uint8_t, std::int64_t, raft::layout_f_contiguous>(
          y.data(), params.n, params.k);
      auto dist_view =
        raft::make_device_matrix_view<OutputType, std::int64_t, raft::layout_f_contiguous>(
          dist_optimized.data(), params.m, params.n);
...
TEST_P(BitwiseHammingTestU32, Result)
{
  ...
  ASSERT_TRUE(cuvs::devArrMatch(dist_ref.data(),
                                dist_optimized.data(),
                                m,
                                n,
                                cuvs::CompareApprox<uint32_t>(params.tolerance),
                                stream))
...
INSTANTIATE_TEST_CASE_P(BitwiseHammingTests,
                        BitwiseHammingTestU32,
