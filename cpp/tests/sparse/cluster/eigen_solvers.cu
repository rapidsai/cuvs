...
TEST(Raft, SpectralPartition)
{
  ...
  auto eig_cfg = cuvs::spectral::eigen_solver_config_t<int, float>{
    n_eig_vects, evs_max_iter, restartIter_lanczos, evs_tolerance, false, seed_eig_solver};
  auto eigen_solver = cuvs::spectral::lanczos_solver_t<int, float>{eig_cfg};

  auto clust_cfg = cuvs::spectral::cluster_solver_config_t<int, float>{
    n_clusters, kmean_max_iter, kmean_tolerance, seed_cluster_solver};
  auto cluster_solver = cuvs::spectral::kmeans_solver_t<int, float>{clust_cfg};
  ...
  cuvs::spectral::analyzePartition(
