...
inline ::std::ostream& operator<<(::std::ostream& os, const AnnHnswAceInputs& p)
{
  os << "{n_queries=" << p.n_queries << ", dataset shape=" << p.n_rows << "x" << p.dim
     ...
     << ", ef_construction=" << p.ef_construction << ", metric=";
  switch (p.metric) {
...
class AnnHnswAceTest : public ::testing::TestWithParam<AnnHnswAceInputs> {
 ...
 protected:
  ...
  void testHnswAceMemoryLimit()
  {
    // This test verifies that ACE works correctly with custom memory limits
    // Create temporary directory for ACE build
...

    {
      ...
      // Configure ACE parameters with memory limits (ACE always uses disk-based storage)
      auto ace_params            = graph_build_params::ace_params();
      ace_params.npartitions     = ps.npartitions;
      ace_params.ef_construction = ps.ef_construction;
      ace_params.build_dir       = temp_dir;
      // Set tiny memory limits to test partition adjustment
      ...
      // Verify that ACE files were created
      std::string graph_file     = temp_dir + "/cagra_graph.npy";
      std::string reordered_file = temp_dir + "/reordered_dataset.npy";

      EXPECT_TRUE(std::filesystem::exists(graph_file)) << "Graph file should exist for ACE build";
...
// Inputs specifically for testing with memory limits
inline std::vector<AnnHnswAceInputs> generate_hnsw_ace_memory_limit_inputs()
{
  return {
    // Test with L2 metric
    {10,    // n_queries
     ...
     100,   // ef_construction
     cuvs::distance::DistanceType::L2Expanded,
     0.0,    // min_recall (not checked in memory limit test)
     0.001,  // max_host_memory_gb (tiny limit)
     0.001}  // max_gpu_memory_gb (tiny limit)
  };
}

const std::vector<AnnHnswAceInputs> hnsw_ace_inputs = generate_hnsw_ace_inputs();
const std::vector<AnnHnswAceInputs> hnsw_ace_memory_limit_inputs =
  generate_hnsw_ace_memory_limit_inputs();
