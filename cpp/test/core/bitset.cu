...
class BitsetTest : public testing::TestWithParam<test_spec_bitset> {
 ...
 public:
  ...
  void run()
  {
    ...
    auto my_bitset = cuvs::core::bitset<bitset_t, index_t>(
      res, raft::make_const_mdspan(mask_device.view()), index_t(spec.bitset_len));
    raft::update_host(bitset_result.data(), my_bitset.data(), bitset_result.size(), stream);

    // calculate the reference
    create_cpu_bitset(bitset_ref, mask_cpu);
    raft::resource::sync_stream(res, stream);
    ASSERT_TRUE(hostVecMatch(bitset_ref, bitset_result, cuvs::Compare<bitset_t>()));
    ...
    /*
    TODO: Disabled bitset::test() for cuvs migration
    my_bitset.test(res, raft::make_const_mdspan(query_device.view()), result_device.view());
    raft::update_host(result_cpu.data(), result_device.data_handle(), result_device.extent(0),
    stream); test_cpu_bitset(bitset_ref, query_cpu, result_ref); raft::resource::sync_stream(res,
    stream); ASSERT_TRUE(hostVecMatch(result_cpu, result_ref, Compare<uint8_t>()));*/
...
using Uint8_32 = BitsetTest<uint8_t, uint32_t>;
TEST_P(Uint8_32, Run) { run(); }
INSTANTIATE_TEST_CASE_P(BitsetTest, Uint8_32, inputs_bitset);
...
using Uint32_64 = BitsetTest<uint32_t, int64_t>;
TEST_P(Uint32_64, Run) { run(); }
INSTANTIATE_TEST_CASE_P(BitsetTest, Uint32_64, inputs_bitset);

using Uint64_64 = BitsetTest<uint64_t, int64_t>;
