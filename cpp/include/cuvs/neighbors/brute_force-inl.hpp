...
struct index : cuvs::neighbors::ann::index {
 public:
  ...
  [[nodiscard]] cuvs::distance::DistanceType metric() const noexcept
  {
    return static_cast<cuvs::distance::DistanceType>((int)raft_index_->metric());
  }

  /** Total length of the index (number of vectors). */
  [[nodiscard]] auto size() const noexcept { return raft_index_->size(); }

  /** Dimensionality of the data. */
  [[nodiscard]] auto dim() const noexcept { return raft_index_->dim(); }
  ...
  /** Dataset norms */
  [[nodiscard]] auto norms() const -> raft::device_vector_view<const T, int64_t, raft::row_major>
  {
    return raft_index_->norms();
  }

  /** Whether or not this index has dataset norms */
  [[nodiscard]] bool has_norms() const noexcept { return raft_index_->has_norms(); }

  [[nodiscard]] T metric_arg() const noexcept { return raft_index_->metric_arg(); }
