...
/**
 * Tag for device-resident owning dataset unions (`device_any_owning_dataset<IdxT>`).
 *
 * The specialization lists several `dataset<..., DataT, IdxT>` alternatives with different `DataT`
 * (float/half/int8/uint8 padded, VPQ codebook element types). There is no single outer `DataT`
 * template parameter for the wrapper: which variant alternative is active is often chosen when
 * loading from disk or wiring ownership.
 */
struct host_any_owning_dataset_container {};
struct device_any_owning_dataset_container {};
/** Tag: non-owning view union (`device_any_dataset_view<DataT, IdxT>`). */
struct host_any_dataset_view_container {};
struct device_any_dataset_view_container {};
...
struct dataset<device_empty_dataset_container, void, IdxT> {
  ...
  {
    return dataset_view<device_empty_dataset_container, void, IdxT>{suggested_dim};
  }
};
...
struct dataset<host_empty_dataset_container, void, IdxT> {
  ...
  {
    return dataset_view<host_empty_dataset_container, void, IdxT>{suggested_dim};
  }
};
...
struct dataset<device_padded_dataset_container, DataT, IdxT> {
  ...
  {
    return dataset_view<device_padded_dataset_container, DataT, IdxT>(data_.view(), dim_);
  }
...
// -----------------------------------------------------------------------------
// padded (host row-major with logical dim vs stride)
// -----------------------------------------------------------------------------
...
struct dataset<host_padded_dataset_container, DataT, IdxT> {
  ...
  {
    return dataset_view<host_padded_dataset_container, DataT, IdxT>(data_.view(), dim_);
  }
  [[nodiscard]] auto data_handle() noexcept -> value_type* { return data_.data_handle(); }
  [[nodiscard]] auto data_handle() const noexcept -> const value_type*
  {
...
template <typename containertype, typename DataT, typename IdxT>
[[nodiscard]] auto vpq_dataset_as_view_impl(dataset<containertype, DataT, IdxT> const* self)
  -> dataset_view<containertype, DataT, IdxT>;
...
struct dataset_view<device_vpq_dataset_container, DataT, IdxT> {
  using index_type  = IdxT;
  using target_type = dataset<device_vpq_dataset_container, DataT, IdxT>;
  ...
  {
    RAFT_EXPECTS(ptr != nullptr, "device_vpq_dataset_view: null target");
  }
...
struct dataset<host_vpq_dataset_container, DataT, IdxT> {
  using index_type = IdxT;
  using math_type  = DataT;
  raft::host_matrix<math_type, uint32_t, raft::row_major> vq_code_book;
  raft::host_matrix<math_type, uint32_t, raft::row_major> pq_code_book;
  raft::host_matrix<uint8_t, index_type, raft::row_major> data;

  dataset(raft::host_matrix<math_type, uint32_t, raft::row_major>&& vq_code_book,
          raft::host_matrix<math_type, uint32_t, raft::row_major>&& pq_code_book,
          raft::host_matrix<uint8_t, index_type, raft::row_major>&& data)
    : vq_code_book{std::move(vq_code_book)},
      pq_code_book{std::move(pq_code_book)},
...
struct dataset_view<host_vpq_dataset_container, DataT, IdxT> {
  using index_type  = IdxT;
  using target_type = dataset<host_vpq_dataset_container, DataT, IdxT>;
  ...
  explicit dataset_view(target_type const* ptr) : target_(ptr)
  {
    RAFT_EXPECTS(ptr != nullptr, "host_vpq_dataset_view: null target");
  }
...
template <typename containertype, typename DataT, typename IdxT>
[[nodiscard]] inline auto vpq_dataset_as_view_impl(dataset<containertype, DataT, IdxT> const* self)
  -> dataset_view<containertype, DataT, IdxT>
{
  return dataset_view<containertype, DataT, IdxT>(self);
}
...
template <typename DataT, typename IdxT>
struct device_any_dataset_view_types {
  using empty_view   = device_empty_dataset_view<IdxT>;
  using vpq_f16_view = device_vpq_dataset_view<half, IdxT>;
  using vpq_f32_view = device_vpq_dataset_view<float, IdxT>;
  using padded_view  = device_padded_dataset_view<DataT, IdxT>;
};

template <typename DataT, typename IdxT>
struct host_any_dataset_view_types {
  using empty_view   = host_empty_dataset_view<IdxT>;
  using vpq_f16_view = host_vpq_dataset_view<half, IdxT>;
  using vpq_f32_view = host_vpq_dataset_view<float, IdxT>;
  using padded_view  = host_padded_dataset_view<DataT, IdxT>;
};

template <typename IdxT>
struct device_any_owning_dataset_types {
  using empty_owning      = device_empty_dataset<IdxT>;
  using padded_f32_owning = device_padded_dataset<float, IdxT>;
  using padded_f16_owning = device_padded_dataset<half, IdxT>;
  using padded_i8_owning  = device_padded_dataset<int8_t, IdxT>;
  using padded_u8_owning  = device_padded_dataset<uint8_t, IdxT>;
  using vpq_f32_owning    = device_vpq_dataset<float, IdxT>;
  using vpq_f16_owning    = device_vpq_dataset<half, IdxT>;
};

template <typename IdxT>
struct host_any_owning_dataset_types {
  using empty_owning      = host_empty_dataset<IdxT>;
  using padded_f32_owning = host_padded_dataset<float, IdxT>;
  using padded_f16_owning = host_padded_dataset<half, IdxT>;
  using padded_i8_owning  = host_padded_dataset<int8_t, IdxT>;
  using padded_u8_owning  = host_padded_dataset<uint8_t, IdxT>;
  using vpq_f32_owning    = host_vpq_dataset<float, IdxT>;
  using vpq_f16_owning    = host_vpq_dataset<half, IdxT>;
};

template <typename IdxT>
struct dataset<device_any_owning_dataset_container, void, IdxT> {
  using index_type = IdxT;
  using owning_variant =
    std::variant<typename device_any_owning_dataset_types<IdxT>::empty_owning,
                 typename device_any_owning_dataset_types<IdxT>::padded_f32_owning,
                 typename device_any_owning_dataset_types<IdxT>::padded_f16_owning,
                 typename device_any_owning_dataset_types<IdxT>::padded_i8_owning,
                 typename device_any_owning_dataset_types<IdxT>::padded_u8_owning,
                 typename device_any_owning_dataset_types<IdxT>::vpq_f32_owning,
                 typename device_any_owning_dataset_types<IdxT>::vpq_f16_owning>;

  owning_variant storage_;

  dataset() = default;

  template <typename containertype, typename DataT>
  explicit dataset(dataset<containertype, DataT, IdxT>&& x) : storage_(std::move(x))
  {
  }
...
template <typename IdxT>
struct dataset<host_any_owning_dataset_container, void, IdxT> {
  using index_type = IdxT;
  using owning_variant =
    std::variant<typename host_any_owning_dataset_types<IdxT>::empty_owning,
                 typename host_any_owning_dataset_types<IdxT>::padded_f32_owning,
                 typename host_any_owning_dataset_types<IdxT>::padded_f16_owning,
                 typename host_any_owning_dataset_types<IdxT>::padded_i8_owning,
                 typename host_any_owning_dataset_types<IdxT>::padded_u8_owning,
                 typename host_any_owning_dataset_types<IdxT>::vpq_f32_owning,
                 typename host_any_owning_dataset_types<IdxT>::vpq_f16_owning>;
...
template <typename DataT, typename IdxT>
struct dataset_view<device_any_dataset_view_container, DataT, IdxT> {
  using index_type = IdxT;
  using variant_type =
    std::variant<typename device_any_dataset_view_types<DataT, IdxT>::empty_view,
                 typename device_any_dataset_view_types<DataT, IdxT>::vpq_f16_view,
                 typename device_any_dataset_view_types<DataT, IdxT>::vpq_f32_view,
                 typename device_any_dataset_view_types<DataT, IdxT>::padded_view>;

  variant_type storage_;

  dataset_view() = default;

  dataset_view(typename device_any_dataset_view_types<DataT, IdxT>::empty_view const& v)
    : storage_(v)
  {
  }
  dataset_view(typename device_any_dataset_view_types<DataT, IdxT>::vpq_f16_view const& v)
    : storage_(v)
  {
  }
  dataset_view(typename device_any_dataset_view_types<DataT, IdxT>::vpq_f32_view const& v)
    : storage_(v)
  {
  }
  dataset_view(typename device_any_dataset_view_types<DataT, IdxT>::padded_view const& v)
    : storage_(v)
  {
  }
...
template <typename DataT, typename IdxT>
struct dataset_view<host_any_dataset_view_container, DataT, IdxT> {
  using index_type   = IdxT;
  using variant_type = std::variant<typename host_any_dataset_view_types<DataT, IdxT>::empty_view,
                                    typename host_any_dataset_view_types<DataT, IdxT>::vpq_f16_view,
                                    typename host_any_dataset_view_types<DataT, IdxT>::vpq_f32_view,
                                    typename host_any_dataset_view_types<DataT, IdxT>::padded_view>;

  variant_type storage_;

  dataset_view() = default;

  dataset_view(typename host_any_dataset_view_types<DataT, IdxT>::empty_view const& v) : storage_(v)
  {
  }
  dataset_view(typename host_any_dataset_view_types<DataT, IdxT>::vpq_f16_view const& v)
    : storage_(v)
  {
  }
  dataset_view(typename host_any_dataset_view_types<DataT, IdxT>::vpq_f32_view const& v)
    : storage_(v)
  {
  }
  dataset_view(typename host_any_dataset_view_types<DataT, IdxT>::padded_view const& v)
    : storage_(v)
  {
  }
...
// -----------------------------------------------------------------------------
// Type-erased / union aliases
// -----------------------------------------------------------------------------

template <typename DataT, typename IdxT>
using device_any_dataset_view = dataset_view<device_any_dataset_view_container, DataT, IdxT>;

template <typename DataT, typename IdxT>
using host_any_dataset_view = dataset_view<host_any_dataset_view_container, DataT, IdxT>;

template <typename IdxT>
using device_any_owning_dataset = dataset<device_any_owning_dataset_container, void, IdxT>;

template <typename IdxT>
using host_any_owning_dataset = dataset<host_any_owning_dataset_container, void, IdxT>;
  ...
  -> device_padded_dataset_view<typename SrcT::value_type, typename SrcT::index_type>
{
  ...
  RAFT_EXPECTS(device_ptr != nullptr,
               "make_device_padded_dataset_view: source must be device-accessible. "
               "Use make_device_padded_dataset() to get an owning copy.");
  RAFT_EXPECTS(
  ...
  -> host_padded_dataset_view<typename SrcT::value_type, typename SrcT::index_type>
{
  ...
  RAFT_EXPECTS(raft::get_device_for_address(src.data_handle()) == -1,
               "make_host_padded_dataset_view: source must be host-accessible. "
               "Use make_host_padded_dataset() to get an owning copy.");
  RAFT_EXPECTS(
    src_stride == required_stride,
  ...
  auto v = raft::make_host_matrix_view(
    const_cast<value_type*>(src.data_handle()), src.extent(0), static_cast<index_type>(src_stride));
  return host_padded_dataset_view<value_type, index_type>(v, src.extent(1));
}
