...
class generic {
   ...
   */
  [[nodiscard]] static inline auto to_string(const T& obj, bool pretty = false) -> std::string
  {
    return to_json(obj).dump(pretty ? 2 : -1);
  }
};
...
{
  return cuvs::core::generic<T>::to_string(obj, pretty);
}
