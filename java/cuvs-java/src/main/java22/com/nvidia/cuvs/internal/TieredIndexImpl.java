...
public class TieredIndexImpl implements TieredIndex {
     ...
     */
    private TieredIndexImpl(TieredIndexParams indexParameters, float[][] vectors,
    ...
    private IndexReference build() throws Throwable {
        try (var localArena = Arena.ofConfined()) {
            ...
            MemorySegment indexParamsMemorySegment = tieredIndexParameters != null
                    ? segmentFromIndexParams(resources, tieredIndexParameters)
                    : MemorySegment.NULL;

            // Get host data
            MemorySegment hostDataSeg = dataset != null ? ((DatasetImpl) dataset).seg :
    ...
    public SearchResults search(TieredIndexQuery query) throws Throwable {
        try (var localArena = Arena.ofConfined()) {
            ...
            returnValue = cudaMemcpy(queriesDP, hostQueriesSeg, queriesBytes, 1); // cudaMemcpyHostToDevice
            checkCudaError(returnValue, "cudaMemcpy");
            ...
            MemorySegment neighborsTensor = prepareTensor(arena, neighborsDP, neighborsShape, 0, 64, 2, 2, 1); // 64-bit int
            ...
            if (query.getPrefilter() != null) {
                ...
                MemorySegment prefilterTensor = prepareTensor(arena, prefilterDP, prefilterShape, 1, 32, 1, 2, 1);
            ...
            // Perform search
            returnValue = cuvsTieredIndexSearch(cuvsRes, segmentFromSearchParams(query.getCagraSearchParameters()),
                    tieredIndexReference.getMemorySegment(), queriesTensor, neighborsTensor, distancesTensor, prefilter);
            checkCuVSError(returnValue, "cuvsTieredIndexSearch");

            // Copy results from device to host
            returnValue = cudaMemcpy(neighborsSeg, neighborsDP, neighborsBytes, 2); // cudaMemcpyDeviceToHost
            checkCudaError(returnValue, "cudaMemcpy");
    ...
    private void performExtend(float[][] extendVectors, Dataset extendDataset) throws Throwable {
        ...
        // Get host data
        MemorySegment hostDataSeg = extendDataset != null ? ((DatasetImpl) extendDataset).seg
        ...
        returnValue = cuvsTieredIndexExtend(cuvsRes, datasetTensor, tieredIndexReference.getMemorySegment());
