# AI Code Review Guidelines - cuVS Python
...
**Context**: cuVS Python layer provides pythonic interfaces for vector search on the GPU.
...
- **Using test classes instead of standalone functions** (cuVS prefers `test_foo_bar()` functions over `class TestFoo`)
...
- [ ] Is the API consistent with existing cuVS patterns?
...
- [ ] Are errors from cuVS C functions being checked with `check_cuvs`?
...
breaks), ignore style preferences. For cuVS Python: memory safety and proper resource lifecycle are paramount.
