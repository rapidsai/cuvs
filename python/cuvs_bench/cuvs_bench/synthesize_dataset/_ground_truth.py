...
- :func:`compute_groundtruth_nprobe` only checks the ``nprobes`` nearest
  clusters per query (IVF-style). This is the cheap path that scales
  to larger datasets.

Queries are generated by adding a small noise jitter to points drawn from
...
def generate_queries(
    ...
    # Small noise jitter to break exact-match ties between query and dataset.
