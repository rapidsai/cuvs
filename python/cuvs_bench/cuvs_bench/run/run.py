...
def prepare_conf_file(dataset_conf: dict, subset_size: Optional[int], count: int, batch_size: int) -> dict:
    """
...
def gather_algorithm_configs(scripts_path: str, configuration: Optional[str]) -> list:
    """
...
def load_algorithms_conf(algos_conf_fs: list, allowed_algos: Optional[list], allowed_algo_groups: Optional[tuple]) -> dict:
    ...
    -------
    dict
        A dictionary containing the loaded and filtered algorithm configurations.
    """
    algos_conf = {}
    for algo_f in algos_conf_fs:
        ...
        if allowed_algos and algo["name"] not in allowed_algos:
            continue
        algos_conf[algo["name"]] = {"groups": algo.get("groups", {}), "constraints": algo.get("constraints", {})}
        if allowed_algo_groups and algo["name"] in allowed_algo_groups[0]:
            algos_conf[algo["name"]]["groups"].update(
                {group: algo["groups"][group] for group in allowed_algo_groups[1] if group in algo["groups"]}
            )
    return algos_conf


def prepare_executables(algos_conf: dict, algos_yaml: dict, gpu_present: bool, conf_file: dict, dataset_path: str, dataset: str, count: int, batch_size: int) -> dict:
    """
    ...
    -------
    dict
        A dictionary of executables to run with their associated configurations.
    """
    executables_to_run = {}
...
def validate_constraints(
    ...
    ------
    ValueError
        If `dims` are needed for build constraints but not specified in the dataset configuration.
    """
...
def run_benchmark(
    ...
    dataset_conf_all = load_yaml_file(dataset_configuration or os.path.join(scripts_path, "../config/datasets", "datasets.yaml"))
    ...
    allowed_algo_groups = [algo_group.split(".") for algo_group in algo_groups.split(",")] if algo_groups else None
    algos_conf = load_algorithms_conf(algos_conf_fs, allowed_algos, list(zip(*allowed_algo_groups)) if allowed_algo_groups else None)

    executables_to_run = prepare_executables(algos_conf, load_yaml_file(os.path.join(scripts_path, "../config", "algorithms.yaml")), gpu_present, conf_file, dataset_path, dataset, count, batch_size)

    cuvs_bench_cpp(
        conf_file,
        dataset,
        os.path.dirname(configuration) if configuration and os.path.isfile(configuration) else os.path.join(scripts_path, "conf", "algos"),
        executables_to_run,
        dataset_path,
        force,
        build,
