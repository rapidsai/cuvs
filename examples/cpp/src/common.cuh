		   ...
		   raft::device_vector_view<int32_t> query_labels)
{
  ...
  auto data_labels_host = raft::make_host_matrix<int32_t, int64_t>(data_labels.extent(0), data_labels.extent(1));
  ...
  raft::copy(query_labels_host.data_handle(), query_labels.data_handle(), query_labels.size(), stream);
  ...
  for (int query_id = 0; query_id < 2; query_id++) {
    std::cout << "Query " << query_id << ", label: ," << query_labels_host(query_id) << ", neighbor:" << std::endl;
    for(int nei = 0; nei < topk; nei++) {
      int neighbor_id = neighbors_host(query_id, nei);
      std::cout << "ID:" << neighbor_id << ", labels: ";
      raft::print_host_vector("", &data_labels_host(neighbor_id, 0), data_labels.extent(1), std::cout);
    }
  }
}
