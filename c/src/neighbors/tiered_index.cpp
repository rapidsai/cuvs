...
void* _build(cuvsResources_t res, cuvsTieredIndexParams params, DLManagedTensor* dataset_tensor)
{
  ...
  switch (params.algo) {
    case CUVS_TIERED_INDEX_ALGO_CAGRA: {
      ...
      return new tiered_index::index<cagra::padded_index<T, uint32_t>>(
  ...
  return cuvs::core::translate_exceptions([=] {
    ...
    if (index.dtype.code == kDLFloat && index.dtype.bits == 32) {
      switch (index.algo) {
        case CUVS_TIERED_INDEX_ALGO_CAGRA: {
          auto index_ptr =
            reinterpret_cast<tiered_index::index<cagra::padded_index<float, uint32_t>>*>(index.addr);
          delete index_ptr;
          break;
        }
        case CUVS_TIERED_INDEX_ALGO_IVF_FLAT: {
...

{
  return cuvs::core::translate_exceptions([=] {
    ...
    switch (index.algo) {
      case CUVS_TIERED_INDEX_ALGO_CAGRA: {
        _search<cagra::padded_index<float, uint32_t>>(
                                             ...
                                             cuvsTieredIndex_t index_c_ptr)
{
  return cuvs::core::translate_exceptions([=] {
    ...
    switch (index.algo) {
      case CUVS_TIERED_INDEX_ALGO_CAGRA: {
        _extend<cagra::padded_index<float, uint32_t>>(res, new_vectors, index);
        break;
      }
      case CUVS_TIERED_INDEX_ALGO_IVF_FLAT: {
                                            ...
                                            cuvsTieredIndex_t output_index)
{
  return cuvs::core::translate_exceptions([=] {
    ...
    switch (indices[0]->algo) {
      case CUVS_TIERED_INDEX_ALGO_CAGRA: {
        _merge<cagra::padded_index<float, uint32_t>>(res, *params, indices, num_indices, output_index);
        break;
      }
      case CUVS_TIERED_INDEX_ALGO_IVF_FLAT: {
