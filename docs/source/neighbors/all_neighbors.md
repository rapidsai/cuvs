...
\[ `C API <../c_api/neighbors_all_neighbors_c>` \| `C++ API <../cpp_api/neighbors_all_neighbors>` \| `Python API <../python_api/neighbors_all_neighbors>` \]

## Algorithm Overview

All-neighbors works by:

1.  **Partitioning**: Dividing the dataset into <span class="title-ref">n_clusters</span> clusters/batches
2.  **Assignment**: Assigning each point to <span class="title-ref">overlap_factor</span> clusters (must be \< n_clusters)
