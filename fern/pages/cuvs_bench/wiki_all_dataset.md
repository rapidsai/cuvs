...
A version of the dataset is made available in the binary format that can be used directly by the [cuvs-bench](/user-guide/benchmarking-guide/cu-vs-bench-tool) tool. The full 88M dataset is ~251GB and the download link below contains tarballs that have been split into multiple parts.
